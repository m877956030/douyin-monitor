...
function cleanTranscript(text: string): string {
  return text
    // 移除所有 Emoji（覆盖基本多文种平面外的表情符号区段）
    .replace(/[\u{1F000}-\u{1FAFF}\u{1F1E6}-\u{1F1FF}\u{2600}-\u{27BF}\u{2B00}-\u{2BFF}\u{FE00}-\u{FE0F}\u{1F900}-\u{1F9FF}\u{1F300}-\u{1F5FF}\u{1F600}-\u{1F64F}\u{1F680}-\u{1F6FF}\u{1F700}-\u{1F77F}\u{1F780}-\u{1F7FF}\u{1F800}-\u{1F8FF}\u{1F200}-\u{1F2FF}\u{2700}-\u{27BF}]/gu, "")
    // 移除模型插入的音乐/音效标记（如 🎼、♪、♫ 等）
    .replace(/[\u2669-\u266D\u266F\u266A\u266B\u266C\u266E]/g, "")
    ...
    // 移除 [音乐] [掌声] [笑声] 等标注
    .replace(/\[(?:音乐|掌声|笑声|咳嗽|呼吸|沉默|噪声|背景音|声音)\]/g, "")
    // 移除 (Music) (Applause) 等英文标注
    .replace(/\((?:Music|Applause|Laughter|Silence|Noise|Background)\)/gi, "")
    // 合并多余空格和换行
    .replace(/\s+/g, " ")
    .trim();
}
...
async function transcribeWithGroq(
  ...
  // 引导 Whisper 生成带标点的中文转录
  formData.append("prompt", "以下是普通话的句子，请使用正确的标点符号。");
...
// ─── 快照保存工具函数 ──────────────────────────────────────
// 批量保存作品指标快照
async function saveWorkSnapshots(
  supabaseUrl: string,
  serviceKey: string,
  bloggerId: string,
  ...
  try {
    // 查出该博主所有作品的 id + douyin_aweme_id
    const res = await fetch(
      `${supabaseUrl}/rest/v1/works?blogger_id=eq.${encodeURIComponent(bloggerId)}&select=id,douyin_aweme_id`,
      { headers: dbHeaders(serviceKey) },
    );
    const rows = await res.json();
    if (!Array.isArray(rows)) return;

    // 建 aweme_id → work_id 映射
    const awemeMap = new Map<string, string>();
    for (const row of rows) {
      if (row.douyin_aweme_id) awemeMap.set(String(row.douyin_aweme_id), row.id);
    }

    const now = new Date().toISOString();
    const snapshots: Record<string, unknown>[] = [];
    for (const w of works) {
      const awemeId = String(w.douyin_aweme_id || w.aweme_id || "");
      const workId = awemeMap.get(awemeId);
      if (!workId) continue;
      snapshots.push({
        work_id: workId,
    ...
    const insertRes = await fetch(`${supabaseUrl}/rest/v1/work_snapshots`, {
      method: "POST",
      headers: { ...dbHeaders(serviceKey), "Content-Type": "application/json", Prefer: "resolution=merge-duplicates" },
      body: JSON.stringify(snapshots),
    });
    if (insertRes.ok) {
      console.log("[saveWorkSnapshots] saved", snapshots.length, "snapshots");
    } else {
      console.log("[saveWorkSnapshots] failed:", insertRes.status, (await insertRes.text()).slice(0, 100));
    }
  } catch (e) {
    console.log("[saveWorkSnapshots] error:", (e as Error).message);
  }
}

// 保存博主指标快照
async function saveBloggerSnapshot(
  supabaseUrl: string,
  serviceKey: string,
  douyinUserId: string,
  ...
  try {
    // 找到 blogger_id
    const res = await fetch(
      `${supabaseUrl}/rest/v1/bloggers?douyin_user_id=eq.${encodeURIComponent(douyinUserId)}&select=id&limit=1`,
      { headers: dbHeaders(serviceKey) },
    );
    const rows = await res.json();
    if (!Array.isArray(rows) || rows.length === 0) return;

    const insertRes = await fetch(`${supabaseUrl}/rest/v1/blogger_snapshots`, {
      method: "POST",
      headers: { ...dbHeaders(serviceKey), "Content-Type": "application/json", Prefer: "resolution=merge-duplicates" },
      body: JSON.stringify({
    ...
    if (insertRes.ok) {
      console.log("[saveBloggerSnapshot] saved for", douyinUserId);
    }
  } catch (e) {
    console.log("[saveBloggerSnapshot] error:", (e as Error).message);
  }
}
...
serve(async (req) => {
  ...
  try {
    ...
    if (action === "get_user_info") {
      ...
      const u = d.user;
      const followerCount = num(u.follower_count);
      const totalWorks = num(u.aweme_count);
      const douyinUserId = u.uid || u.sec_uid;

      // 保存博主指标快照
      const SUPABASE_URL = Deno.env.get("SUPABASE_URL") || "";
      const SERVICE_KEY = Deno.env.get("SUPABASE_SERVICE_ROLE_KEY") || Deno.env.get("SUPABASE_ANON_KEY") || "";
      await saveBloggerSnapshot(SUPABASE_URL, SERVICE_KEY, douyinUserId, followerCount, totalWorks);
    ...
    if (action === "get_subtitle") {
      ...
      if (freshPlayUrl && (sfKey || groqKey)) {
        // Groq 限制 25MB；只有硅基流动时，提高到 50MB 以避免文案被截断
        // 如果两者都有，优先用硅基流动的 50MB（更完整），但 Groq 会先尝试（更快）
        // 实际策略：用 50MB 下载完整音频，Groq 失败时硅基流动仍能用完整数据
        const maxDownload = 50 * 1024 * 1024;
    ...
    if (action === "get_works") {
      ...
      if ((apiFailed || works.length === 0) && sec_uid) {
        ...
        if (htmlWorks.length > 0) {
          ...
          const SUPABASE_URL = Deno.env.get("SUPABASE_URL") || "";
          const SERVICE_KEY = Deno.env.get("SUPABASE_SERVICE_ROLE_KEY") || Deno.env.get("SUPABASE_ANON_KEY") || "";
          const bloggerId = String(body.blogger_id || "");
          if (bloggerId) {
            await saveWorkSnapshots(SUPABASE_URL, SERVICE_KEY, bloggerId, htmlWorks);
          }
      ...
      if (works.length > 0) {
        ...
        const bloggerId = String(body.blogger_id || "");
        if (bloggerId) {
          await saveWorkSnapshots(SUPABASE_URL, SERVICE_KEY, bloggerId, works);
        }
      }
