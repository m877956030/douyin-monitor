...
> 本次改版新增 `react-native-webview`（用于 AI 配音工具内嵌网页），首次安装前请执行：
> ```bash
> pnpm install
> # 或
...
| 功能 | 说明 |
|------|------|
| 博主管理 | 添加博主主页链接，自动获取最新作品 |
| 分组管理 | 首页「我的分组」展示分组，点击进入分组 → 博主 → 作品 |
| 监控 | 底部 Tab「监控」展示今日所有博主最新发布作品 |
| 文案获取 | 优先字幕 → 硅基流动音频转录 → 视频描述 |
| 时长显示 | 作品列表中在发布时间旁显示视频时长 |
| AI 分析 | 18 维度文风深度分析（浅色主题） |
| AI 对话 | 基于当前文案的对话，支持 @引用其他作品、写作模板 |
| AI 配音 | 接入 VoxCPM2 与 IndexTTS2，支持 Cookie 登录后使用 |
| 模型切换 | 对话界面顶部可直接切换 AI 提供商 |

## 八、本次改动摘要

- 底部 Tab 改为：博主 / 监控 / AI工具 / 设置
- 原「分类」Tab 改为「监控」，展示当日最新作品
- 分类管理功能移至首页「我的分组」→「管理」
- 分组详情：点击分组查看博主，点击博主查看其作品
- 作品列表与详情页标题不再截断，完整显示
- 作品列表新增时长显示（发布时间旁）
- 删除作品详情中的「脑图」与「高赞评论」Tab
- 新增 AI 工具页，支持 VoxCPM2 / IndexTTS2 配音工具 Cookie 登录与内嵌使用
